<?php
class QCLD_mailing_list_AutoUpdate 
{
	/**
	...
	{
		$qcld_renew_subscription = get_mailing_list_renew_transient();
	...
	public function check_update( $transient )
	{
		...
		if ( version_compare( $this->current_version, $remote_version->new_version, '<' ) ) {
			...
			if( $this->slug == $subscription_enable ){
				...
				set_mailing_list_update_transient($obj);
				delete_mailing_list_renew_transient();
				
				$transient->response[$this->plugin_slug] = $obj;
			}else{
			 	$obj = new stdClass();
			 	set_mailing_list_renew_transient($obj);
			 	$qcld_renew_subscription = get_mailing_list_renew_transient();
	  		...
	  		echo '<tr class="plugin-update-tr">
	  			<td colspan="3" class="plugin-update colspanchange">
		        	<div class="update-message notice inline notice-warning notice-alt">
		        		<p>There is a new version available. <a href="'.esc_url('https://www.ultrawebmedia.com/li/plugins/qc-mailing-list-integration-addon/changelog.txt').'" target="_blank">View version details</a>. Automatic update is unavailable for this plugin. To receive automatic updates, valid license is required.</p>
		        	</div>
		        </td>
		    </tr>';
		}
