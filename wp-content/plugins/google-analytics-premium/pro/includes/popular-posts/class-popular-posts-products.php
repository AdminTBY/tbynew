...
class MonsterInsights_Popular_Posts_Products extends MonsterInsights_Popular_Posts {
	...
	public function get_rendered_html( $atts ) {
		...
		foreach ( $posts as $post ) {
			...
			$html .= '<span class="monsterinsights-products-popular-posts-title" ' . esc_attr($this->get_element_style( $theme, 'title', $atts )) . '>' . esc_html($post['title']) . '</span>';
			if ( $show_price ) {
				$html .= '<span class="monsterinsights-products-popular-posts-price" ' . esc_attr($this->get_element_style( $theme, 'price', $atts )) . '>' . wp_kses_post($post['price']) . '</span>';
			}
			if ( $show_rating ) {
		...
		foreach ( $themes as $theme_key => $theme_styles ) {
			...
			if ( ! empty( $theme_styles['price'] ) ) {
				...
				if ( ! empty( $theme_styles['price']['color'] ) ) {
					$styles .= 'color:' . esc_attr($theme_styles['price']['color']) . ';';
				}
				if ( ! empty( $theme_styles['price']['size'] ) ) {
					$styles .= 'font-size:' . esc_attr($theme_styles['price']['size']) . 'px;';
				}
	...
	public function add_inline_posts_to_content() {
		echo $this->shortcode_output( array() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	...
	public function remove_widget_from_legacy_widgets( $widgets ) {
		$widgets[] = 'monsterinsights-popular-posts-products';
