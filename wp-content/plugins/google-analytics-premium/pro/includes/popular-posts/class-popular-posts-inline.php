...
class MonsterInsights_Popular_Posts_Inline extends MonsterInsights_Popular_Posts {
	...
	public function get_rendered_html( $atts ) {
		...
		if ( ! empty( $theme_styles['image'] ) && ! empty( $posts[0]['image'] ) ) {
			...
			$html .= '<a href="' . esc_url($posts[0]['link']) . '"><img src="' . esc_url($posts[0]['image']) . '" srcset=" ' . esc_attr($posts[0]['srcset']) . ' " alt="' . esc_attr( $posts[0]['title'] ) . '" /></a>';
			$html .= '</div>';
		}
		...
		foreach ( $posts as $post ) {
			...
			$html .= '<a class="monsterinsights-inline-popular-posts-title" ' . esc_attr($this->get_element_style( $theme, 'title', $atts )) . ' href="' . $post['link'] . '">' . esc_html($post['title']) . '</a>';
			$html .= '</div>';
		}
		...
		foreach ( $themes as $theme_key => $theme_styles ) {
			...
			if ( ! empty( $theme_styles['icon'] ) ) {
				...
				if ( ! empty( $theme_styles['icon']['color'] ) ) {
					$styles .= 'fill:' . esc_attr($theme_styles['icon']['color']) . ';';
				}
				...
				if ( ! empty( $theme_styles['icon']['background'] ) ) {
					...
					$styles .= 'background-color:' . esc_attr($theme_styles['icon']['background']) . ';';
					$styles .= '}';
				}
			}
		}
