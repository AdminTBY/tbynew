			...
			foreach ( $order_ids as $order_id ) {
				...
				if ( count( $deal_updates ) ) {
					...
					if ( $flag ) {
						...
						$user_detail['first_name'] = get_post_meta( $order_id, '_billing_first_name', true );
						$user_detail['last_name']  = get_post_meta( $order_id, '_billing_last_name', true );
						...
						if ( 200 == $response['status_code'] ) {
							...
							if ( ! empty( $result ) ) {
								foreach ( $result as $key => $value ) {
									...
									update_post_meta( $order_id, 'hubwoo_order_line_item_created', 'yes' );
								}
							}
						}
			...
			if ( ! empty( $order_id ) ) {
				...
				if ( is_array( $order_items ) && count( $order_items ) ) {
					...
					foreach ( $order_items as $item_key => $single_item ) :
						...
						if ( ! empty( $line_item_hs_id ) || 'yes' == get_post_meta( $order_id, 'hubwoo_order_line_item_created', 'no' ) ) {
							continue;
						}
