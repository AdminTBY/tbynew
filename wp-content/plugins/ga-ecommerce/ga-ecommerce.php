...
class MonsterInsights_eCommerce {
	...
	public function require_frontend() {
		...
		require_once plugin_dir_path( __FILE__ ) . 'includes/frontend/class-memberpress-tracking.php';
		require_once plugin_dir_path( __FILE__ ) . 'includes/frontend/class-lifterlms-ecommerce-tracking.php';
		require_once plugin_dir_path( __FILE__ ) . 'includes/frontend/class-rcp-ecommerce-tracking.php';
		require_once plugin_dir_path( __FILE__ ) . 'includes/frontend/class-givewp-ecommerce-tracking.php';
		...
		$tracking_code_ua  = monsterinsights_get_ua();
		$tracking_code_v4  = function_exists( 'monsterinsights_get_v4_id' ) && monsterinsights_get_v4_id();
		$has_tracking      = ! empty( $tracking_code_ua ) || ! empty( $tracking_code_v4 );
		$enhanced_commerce = (bool) monsterinsights_get_option( 'enhanced_ecommerce', false ) && version_compare( MONSTERINSIGHTS_VERSION, '7.0.0', '>' );
		...
		} else if ( $has_tracking && $enhanced_commerce ) {
			if ( class_exists( 'Easy_Digital_Downloads' ) && apply_filters( 'monsterinsights_track_edd', true ) ) {
	...
	public function deactivate_plugins() {
		if ( function_exists( 'MonsterInsights' ) ) {
			if ( MonsterInsights()->auth->get_ua() ) {
