...
class MonsterInsights_eCommerce_LifterLMS_Integration extends MonsterInsights_Enhanced_eCommerce_Integration {
	...
	private function track_purchase_ua( $order, $order_id, $txn ) {
		...
		$atts = array(
			...
			'ev'  => round( $txn->get( 'amount' ) * 100 ),                // Event Value
			'cos' => 2,                                                    // Checkout Step
			'pa'  => $this->get_funnel_action( 'completed_purchase' ),    // Product Action
			'cid' => monsterinsights_get_client_id( $order_id ),          // GA Client ID
			'ti'  => $txn->get( 'id' ),                                    // Transaction ID
			'ta'  => null,                                                // Affiliation
			'tr'  => $txn->get( 'amount' ),                                // Revenue
			'tt'  => 0,                                                    // Taxes
			'ts'  => 0,                                                    // Shipping
			'tcc' => $order->get( 'coupon_code' ),                        // Discount code
			'cu'  => $order->get( 'currency' ),                           // Currency
		);

		if ( monsterinsights_get_option( 'userid', false ) ) {
			$atts['uid'] = $order->get( 'user_id' ); // UserID tracking
		}

		$atts = array_merge( $atts, $this->get_cart_items( $order->get( 'plan_id' ) ) );
	...
	public function add_order( $txn ) {
		...
		$this->track_purchase_ua( $order, $order_id, $txn );
	...
	private function track_refund_ua( $order, $order_id, $txn ) {
		if ( ! monsterinsights_get_ua_to_output() ) {
			return;
		}
		...
		$atts = array(
			...
			'el'  => $order_id,                                    // Event Label
			'ev'  => - 1 * round( $txn->get( 'refund_amount' ) * 100 ),      // Event Value
			'cid' => monsterinsights_get_client_id( $order_id ),   // GA Client ID
		);
		...
		// Partial Refund: Declare items in cart
		if ( $txn->get_refundable_amount() > 0 ) {

			$items = array(
				'pr1id' => $order->get( 'product_id' ), // Product ID
				'pr1qt' => 1,
			);
	...
	public function remove_order( $txn ) {
		...
		$this->track_refund_ua( $order, $order_id, $txn );
	...
	private function track_begin_checkout_ua( $plan ) {
		if ( ! monsterinsights_get_ua_to_output() ) {
			return;
		}
		...
		$atts = array_merge( $atts, $this->get_cart_items( $plan ) );
	...
	public function do_step_started_checkout() {
		...
		$this->track_begin_checkout_ua( $plan );
