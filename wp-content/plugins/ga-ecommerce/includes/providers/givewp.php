...
class MonsterInsights_eCommerce_GiveWP_Integration extends MonsterInsights_Enhanced_eCommerce_Integration {
    ...
    private function track_purchase_ua( $donation_id, $total, $payment_id, $currency, $donor_id ) {
		...
		$atts = array(
			...
			'el'  => $donation_id,                                         // Event Label
			'ev'  => round( $total * 100 ),                                // Event Value
			'cos' => 1,                                                    // Checkout Step
		...
		if ( monsterinsights_get_option( 'userid', false ) ) {
			$atts['uid'] = $donor_id; // UserID tracking
		}
	...
	public function add_order( $payment_id ) {
		...
		$this->track_purchase_ua( $donation_id, $total, $payment_id, $currency, $donor_id );
	...
	private function track_refund_ua( $donation_id, $total, $payment_id, $donor_id ) {
		if ( ! monsterinsights_get_ua_to_output() ) {
			return;
		}
		...
		$atts = array(
			...
			'el'  => $donation_id,                                            // Event Label
			'ev'  => - 1 * round( $total * 100 ),                             // Event Value
			'cid' => monsterinsights_get_client_id( $payment_id ),            // GA Client ID
		);
		...
		$items          = array();
		$items["pr1id"] = give_get_payment_form_id( $payment_id );                                            // Product ID
		$items["pr1nm"] = give_get_payment_meta( $payment_id, '_give_payment_form_title', true );                // Product Name
		$items["pr1pr"] = $total;                                                                                // Product Price
	...
	public function remove_order( $payment_id, $new_status, $old_status ) {
		...
		$this->track_refund_ua( $donation_id, $total, $payment_id, $donor_id );
