...
class MonsterInsights_eCommerce_EDD_Integration extends MonsterInsights_Enhanced_eCommerce_Integration {
	...
	private function get_tracked_item_ua( $download_id, $item_options, $override_quantity = null, $override_price = null, $index = 1 ) {
		$item = $this->get_tracked_item_info( $download_id, $item_options, $override_quantity, $override_price );

		return array(
			"pr{$index}id" => $item['item_id'],
			"pr{$index}nm" => $item['item_name'],
			"pr{$index}ca" => $item['item_category'],
			"pr{$index}va" => $item['item_variant'],
			"pr{$index}pr" => $item['price'],
			"pr{$index}qt" => $item['quantity'],
		);
	}

	private function track_add_to_cart_ua( $download_id, $cart_options, $override_quantity = null, $action = 'Add to Cart' ) {
		if ( ! monsterinsights_get_ua_to_output() ) {
			return;
		}

		$item = $this->get_tracked_item_ua( $download_id, $cart_options, $override_quantity );
		...
		$atts = array_merge(
			array(
				...
				'pal' => '',                                        // Product Action
				'el'  => htmlentities( $item['pr1nm'], ENT_QUOTES, 'UTF-8' ),
				'ev'  => $item['pr1qt'],
			),
			$item
		);
	...
	public function add_to_cart( $download_id, $options ) {
		...
		$this->track_add_to_cart_ua( $download_id, $options );
	...
	public function change_cart_quantity() {
		...
		if ( $diff > 0 ) {
			$this->track_add_to_cart_ua( $download_id, $options, $diff, 'Increased Cart Quantity' );
			$this->track_add_to_cart_v4( $download_id, $options, $diff );
		} elseif ( $diff < 0 ) {
			$this->track_remove_from_cart_ua( $download_id, $options, - $diff, 'Decreased Cart Quantity' );
			$this->track_remove_from_cart_v4( $download_id, $options, - $diff );
		}
	}

	private function track_remove_from_cart_ua( $download_id, $cart_options, $override_quantity = null, $action = 'Remove From Cart' ) {
		if ( ! monsterinsights_get_ua_to_output() ) {
			return;
		}
	...
	public function remove_from_cart( $cart_key ) {
		...
		$this->track_remove_from_cart_ua( $cart_contents[ $cart_key ]['id'], $cart_contents[ $cart_key ]['options'] );
		$this->track_remove_from_cart_v4( $cart_contents[ $cart_key ]['id'], $cart_contents[ $cart_key ]['options'] );
	}

	private function get_tracked_cart_items_ua( $cart_contents, $is_checkout = false ) {
		...
		foreach ( $cart_contents as $key => $item ) {
			...
			$ua_item             = $this->get_tracked_item_ua( $item['id'], $item['item_number']['options'], $item['quantity'], $override_price, $i );
			$ua_item["pr{$i}ps"] = $i;
	...
	private function track_checkout_ua( $cart_contents ) {
		if ( ! monsterinsights_get_ua_to_output() ) {
			return;
		}
		...
		$atts = array_merge(
			$atts,
			$this->get_tracked_cart_items_ua( $cart_contents )
		);
		monsterinsights_mp_track_event_call( $atts );
	}
	...
	public function checkout_page() {
		...
		$this->track_checkout_ua( $cart_contents );
	...
	private function track_purchase_ua( $payment_id, $discount, $cart_contents ) {
		if ( ! monsterinsights_get_ua_to_output() ) {
			return;
		}
		...
		$atts = array(
			...
			'el'             => $payment_id,
			// Event Label
			'ev'             => round( edd_get_payment_amount( $payment_id ) * 100 ),
			// Event Value
			'cos'            => '2',
			// Checkout Step
			'pa'             => $this->get_funnel_action( 'completed_purchase' ),
			// Product Action
			...
			'ta'             => $affiliate_id,
			// Affiliation
			'tr'             => edd_get_payment_amount( $payment_id ),
			// Revenue
			'tt'             => edd_use_taxes() ? edd_get_payment_tax( $payment_id ) : null,
			// Taxes
			'ts'             => null,
			// Shipping
			'tcc'            => $discount,
		...
		$atts = array_merge(
			$atts,
			$this->get_tracked_cart_items_ua( $cart_contents, true )
		);
		monsterinsights_mp_track_event_call( $atts );
	}
	...
	public function add_order( $payment_id, $new_status, $old_status ) {
		...
		$this->track_purchase_ua( $payment_id, $discount, $cart_contents );
		$this->track_purchase_v4( $payment_id, $discount, $cart_contents );

		update_post_meta( $payment_id, '_monsterinsights_is_in_ga', 'yes' );
	}

	private function track_refund_ua( $payment_id, $cart_contents ) {
		if ( ! monsterinsights_get_ua_to_output() ) {
			return;
		}
		...
		$atts = array(
			...
			'el'             => $payment_id,
			// Event Label
			'ev'             => - 1 * round( edd_get_payment_amount( $payment_id ) * 100 ),
			// Event Value
			'pa'             => 'refund',
			// Product Action
			'cid'            => monsterinsights_get_client_id( $payment_id ),
	...
	public function remove_order( $payment_id, $new_status, $old_status ) {
		...
		$this->track_refund_ua( $payment_id, $cart_contents );
