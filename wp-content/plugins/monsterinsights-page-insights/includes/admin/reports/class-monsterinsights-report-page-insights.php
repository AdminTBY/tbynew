...
final class MonsterInsights_Report_Page_Insights extends MonsterInsights_Report {
	...
	private static function is_ua() {
		return 'ua' === MonsterInsights()->auth->get_connected_type();
	}
	...
	public static function get_default_metrics_value() {
		...
		if ( ! self::is_ua() ) {
			unset( $defaults['bouncerate'], $defaults['pageloadtime'] );
		}
	...
	public static function get_metrics_labels() {
		...
		if ( ! self::is_ua() ) {
			unset( $labels['bouncerate'], $labels['pageloadtime'] );
		}
