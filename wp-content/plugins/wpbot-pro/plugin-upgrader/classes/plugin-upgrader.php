<?php
class QCLD_wpbotpro_AutoUpdate 
{
	/**
	...
	public function check_update( $transient )
	{
		...
		if ( version_compare( $this->current_version, $remote_version->new_version, '<' ) ) {
			...
			//error_log('subscription_enable '.var_dump($remote_version));
			...
			if( $this->slug == $subscription_enable ){
				...
				set_wpbotpro_update_transient($obj);
				...
				$transient->response[$this->plugin_slug] = $obj;
			 	...
			 	$obj = new stdClass();
			 	set_wpbotpro_renew_transient($obj);
			 	$qcld_renew_subscription = get_wpbotpro_renew_transient();
			 	if( $qcld_renew_subscription ){
			 		add_action("after_plugin_row_{$this->plugin_slug}", array($this, 'show_upgrade_subscription_on_plugin_row'), 10, 3 );
			 	}
	...
	{
		$qcld_update_plugin = get_wpbotpro_update_transient();

		// if( $qcld_update_plugin ){
			
		// 	return maybe_unserialize($qcld_update_plugin);
		// }else{
			...
			$request = wp_remote_post($this->update_path, $params, array('timeout' => 300) );
